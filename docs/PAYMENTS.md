...
- Clock payment/refund posting rereads the target folio before and after the credit item and stores raw Clock balances in `balance_before`, `expected_balance`, `actual_balance`, and `reconciliation_status`. For `deposit=true` folios, a positive website deposit is expected to move Clock's raw folio balance negative, so verification compares the signed raw balance and the normalized deposit amount held instead of treating `+150` and `-150` as equivalent.

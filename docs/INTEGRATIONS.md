...
- The official Clock API cache exposes booking charge creation and folio void events, but this task found no safe documented item-level accommodation-charge reversal or cancellation-fee posting sequence. Cancellation records therefore use `manual_clock_charge_cleanup_required` and `manual_clock_cancellation_fee_required` instead of guessing an API write.

<?php

namespace MustHotelBooking\Admin;

final class AvailabilityAdminQuery
{
    private string $action;
    private int $ruleId;
    private int $blockId;
    private int $roomId;
    private string $status;
    private string $timeline;
    private string $mode;
    private string $ruleType;
    private string $search;
    private string $startDate;
    private string $endDate;
    private string $notice;

    /**
     * @param array<string, mixed> $request
     */
    public static function fromRequest(array $request): self
    {
        return new self(
            isset($request['action']) ? \sanitize_key((string) $request['action']) : '',
            isset($request['rule_id']) ? \absint($request['rule_id']) : 0,
            isset($request['block_id']) ? \absint($request['block_id']) : 0,
            isset($request['room_id']) ? \absint($request['room_id']) : 0,
            isset($request['status']) ? \sanitize_key((string) $request['status']) : '',
            isset($request['timeline']) ? \sanitize_key((string) $request['timeline']) : '',
            isset($request['mode']) ? \sanitize_key((string) $request['mode']) : '',
            isset($request['rule_type']) ? \sanitize_key((string) $request['rule_type']) : '',
            isset($request['search']) ? \sanitize_text_field((string) $request['search']) : '',
            isset($request['start_date']) ? \sanitize_text_field((string) $request['start_date']) : '',
            isset($request['end_date']) ? \sanitize_text_field((string) $request['end_date']) : '',
            isset($request['notice']) ? \sanitize_key((string) $request['notice']) : ''
        );
    }

    public function __construct(
        string $action,
        int $ruleId,
        int $blockId,
        int $roomId,
        string $status,
        string $timeline,
        string $mode,
        string $ruleType,
        string $search,
        string $startDate,
        string $endDate,
        string $notice
    ) {
        $this->action = $action;
        $this->ruleId = $ruleId;
        $this->blockId = $blockId;
        $this->roomId = $roomId;
        $this->status = $status;
        $this->timeline = $timeline;
        $this->mode = $mode;
        $this->ruleType = $ruleType;
        $this->search = $search;
        $this->startDate = $startDate;
        $this->endDate = $endDate;
        $this->notice = $notice;
    }

    public function getAction(): string
    {
        return $this->action;
    }

    public function getRuleId(): int
    {
        return $this->ruleId;
    }

    public function getBlockId(): int
    {
        return $this->blockId;
    }

    public function getRoomId(): int
    {
        return $this->roomId;
    }

    public function getStatus(): string
    {
        return $this->status;
    }

    public function getTimeline(): string
    {
        return $this->timeline;
    }

    public function getMode(): string
    {
        return $this->mode;
    }

    public function getRuleType(): string
    {
        return $this->ruleType;
    }

    public function getSearch(): string
    {
        return $this->search;
    }

    public function getStartDate(): string
    {
        return $this->startDate;
    }

    public function getEndDate(): string
    {
        return $this->endDate;
    }

    public function getNotice(): string
    {
        return $this->notice;
    }

    /**
     * @param array<string, scalar> $overrides
     * @return array<string, scalar>
     */
    public function buildUrlArgs(array $overrides = []): array
    {
        $args = [
            'room_id' => $this->roomId,
            'status' => $this->status,
            'timeline' => $this->timeline,
            'mode' => $this->mode,
            'rule_type' => $this->ruleType,
            'search' => $this->search,
            'start_date' => $this->startDate,
            'end_date' => $this->endDate,
        ];

        $merged = \array_merge($args, $overrides);

        foreach ($merged as $key => $value) {
            if ($value === '' || $value === 0 || $value === 'all') {
                unset($merged[$key]);
            }
        }

        return $merged;
    }
}

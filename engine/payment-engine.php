...
function get_checkout_payment_methods(): array
{
    ...
    foreach ($preferred_order as $method) {
        ...
        if ($method_key === 'stripe' && !is_stripe_checkout_configured()) {
            continue;
        }
...
function create_stripe_checkout_session(array $reservation_ids, array $guest_form, float $total_amount, string $currency, array $coupon_ids = []): array
{
    ...
    $hotel_name = \class_exists(__NAMESPACE__ . '\MustBookingConfig')
...
function get_payment_reservation_rows(array $reservation_ids): array
{
    ...
    $ids_sql = \implode(', ', \array_fill(0, \count($reservation_ids), '%d'));
    $sql = $wpdb->prepare(
        "SELECT id, booking_id, room_id, guest_id, checkin, checkout, status, total_price, payment_status
...
function create_or_update_payment_rows(array $reservation_ids, string $method, string $status, string $transaction_id = ''): void
{
    ...
    $payments_table = get_payments_table_name();
    ...
    foreach ($reservation_rows as $reservation_row) {
        ...
        $existing_payment_id = (int) $wpdb->get_var(
            $wpdb->prepare(
                "SELECT id
                FROM {$payments_table}
        ...
        if ($existing_payment_id > 0) {
            ...
            $update_formats = ['%f', '%s', '%s', '%s', '%s'];

            if (!isset($payment_data['transaction_id'])) {
                $update_formats = ['%f', '%s', '%s', '%s'];
            }

            if (isset($payment_data['paid_at'])) {
                $update_formats[] = '%s';
            }

            $wpdb->update($payments_table, $payment_data, ['id' => $existing_payment_id], $update_formats, ['%d']);
        ...
        $wpdb->insert($payments_table, $payment_data, $insert_formats);
    }
}
...
function cleanup_expired_pending_payment_reservations(): void
{
    ...
    $reservation_ids = (array) $wpdb->get_col(
        $wpdb->prepare(
            "SELECT id
            FROM {$reservations_table}
            WHERE status = %s
